...
from pymilvus.model.hybrid import BGEM3EmbeddingFunction  #多功能 embedding 的模型，可以同时输出稠密和稀疏向量
# from pymilvus.model.reranker import BGERerankFunction  #没有使用到，当前排序用的RRFRanker
...
# 连接MongoDB、Milvus、Embed
mongo_collection = MongoConfig.get_collection("manual_text")
connections.connect(uri=milvus_db_path)
embedding_handler = BGEM3EmbeddingFunction(model_name=bge_m3_model_path, device="cuda") #使用GPU推理
...
class MilvusRetriever:
    ...
    def save_vectorstore(self, docs: list[str]): 
        ...
        # batch embedding 插入,分batch的原因是embedding计算和milvus插入都支持批量操作，可以提升效率
    ...
    def hybrid_search(
       ...
       #这里稠密和稀疏检索直接使用的AnnSearchRequest的，而不是自定义的
        ...
        rerank = RRFRanker()
        #从两个不同的相关性定义角度，对同一批文档进行打分
        #稠密检索limit个，稀疏检索limit个，然后去重融合，排序后返回topk个
        #res = [   hits_for_query_ ]
    ...
    def retrieve_topk(self, query, topk=10):
        ...
        hybrid_results = self.hybrid_search(
            query_embeddings["dense"][0], #取第0条数据，变成一维向量
            query_embeddings["sparse"][[0]], #取第0条数据，强调保留二维矩阵的格式
            sparse_weight=0.7,
            dense_weight=1.0,
            limit=topk
        )[0]
