...
#这个是单纯根据token重叠程度判断，对轻微改写比较宽容，实现简单稳定
...
#计算两个 token 序列的最长公共子序列长度，衡量顺序相似度的基础量
...
def rouge_l_f1_score(expected: str, predicted: str) -> float:
    ...
    #预测答案中，有多少比例属于和标准答案顺序一致的公共子序列
    precision = lcs / len(predicted_tokens)
    #标准答案中，有多少比例被预测答案按顺序覆盖到了
...
class LocalQwenGenerator:
    ...
    #这一步才是真正拼接成完整的给大模型的内容，并且调用大模型，并得到返回的内容
    @torch.no_grad()
...
def evaluate_predictions(records: Sequence[PredictionRecord]) -> Dict[str, Any]:
    ...
    return {
        ...
        "false_no_answer_count": false_no_answer,   #本来可答却拒答的次数
        "missed_no_answer_count": missed_no_answer,  #该拒答却没拒答的次数
        "exact_match_count": int(sum(row.exact_match for row in records)), #严格匹配成功的样本总数
        "token_f1_std": statistics.pstdev(token_f1_values) if total > 1 else 0.0,
        "status_distribution": status_distribution,
    }


#构建 run_id，格式为：{timestamp}_{custom}_{mode}_{model_name}_{dataset_name}
...
#构建 Markdown 格式的评测报告，包含总体指标和部分最佳/最差样例展示
...
def main() -> None:
    ...
    for index, sample in enumerate(samples, start=1):
        ...
        record = PredictionRecord(
            ...
            answerable_expected=not is_no_answer_text(sample.expected_answer), #标准答案是否可答
            answerable_predicted=not is_no_answer_text(predicted_answer),      #预测答案是否可答
